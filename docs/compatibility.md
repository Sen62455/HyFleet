...
installer does not download, upgrade, or replace sing-box. It verifies the
...
flow `xtls-rprx-vision`. WebSocket, gRPC, HTTPUpgrade, arbitrary sing-box JSON,
traffic accounting, online-user state, kicking, and quota enforcement are not
part of this experimental contract.
...
- Experimental Reality core: non-root account `hyfleet-singbox`, configuration
