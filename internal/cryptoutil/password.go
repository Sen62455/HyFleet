...
func HashPassword(password string, params PasswordParams) (string, error) {
	...
	hash := argon2.IDKey(
		[]byte(password), salt, params.Iterations, params.MemoryKiB,
		params.Parallelism, params.KeyBytes,
	)
	return fmt.Sprintf(
...
func VerifyPassword(encoded, password string) (bool, error) {
	...
	if memory < 8*1024 || memory > 256*1024 || iterations < 1 || iterations > 10 || parallelism < 1 || parallelism > 8 {
		return false, errors.New("password parameters outside accepted bounds")
	}
